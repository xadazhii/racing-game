...
namespace PROMETEO___Car_Controller.Scripts
{
  public class PrometeoCarController : MonoBehaviour
  {
    ...
    //[Header("CAR SETUP")]
    [Space(10)]
    // МИ ПРИБРАЛИ [Range], ТЕПЕР ТУТ МОЖНА ПИСАТИ БУДЬ-ЩО
    public int maxSpeed = 90; // Вписуйте сюди хоч 300, хоч 500
      
    [Range(10, 200)]
    public int maxReverseSpeed = 45; 
      
    // ТУТ ТЕЖ ПРИБРАЛИ ОБМЕЖЕННЯ
    public int accelerationMultiplier = 2; // Вписуйте 20, 30 або більше
    ...
    [Space(10)]
    // І ТУТ ПРИБРАЛИ ОБМЕЖЕННЯ ДЛЯ ГАЛЬМ
    ...
    [Space(20)]
    [Header("NITRO SETUP")]
    public int nitroMultiplier = 3; // У скільки разів пришвидшувати, коли натиснуто Shift
    public ParticleSystem nitroLeftParticles; // Ефект з лівої труби
    public ParticleSystem nitroRightParticles; // Ефект з правої труби
    private bool isNitroOn = false; // Чи ввімкнене нітро зараз
    ...
    void Start()
    {
      ...
      // НОВЕ: Зупинити нітро ефекти на старті
    ...
    void Update()
    {
      ...
      */
      ...
      // НОВЕ: Перевірка Нітро (на ПК це Left Shift)
      CheckNitro();
    ...
    // НОВЕ: Функція перевірки нітро
    void CheckNitro() 
    {
      // Якщо натиснуто лівий Shift - нітро активне
      ...
      if(useSounds){
        try{
          if(carEngineSound != null){
            ...
            // НОВЕ: Якщо нітро, звук стає трохи вищим
            if (isNitroOn) {
              carEngineSound.pitch += 0.5f;
            }
          }
    ...
    public void GoForward(){
      ...
      if(localVelocityZ < -1f){
        ...
        if(Mathf.RoundToInt(carSpeed) < maxSpeed){
          ...
          // НОВЕ: Розрахунок сили прискорення з урахуванням Нітро
          float speedFactor = (accelerationMultiplier * 50f) * throttleAxis;
          if (isNitroOn) {
            speedFactor *= nitroMultiplier; // Множимо на силу нітро
          }
    ...
    // НОВЕ: Функція для керування ефектами машини (включаючи нітро)
    private void CarEffects()
    {
      // Керування димом від коліс (існуюче)
      DriftCarPS();
        
      // Керування частинками Нітро
      if (isNitroOn) 
      {
